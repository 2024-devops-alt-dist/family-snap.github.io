    ...
    caches.open(CACHE_NAME).then(async (cache) => {
      for (const url of urlsToCache) {
        try {
          ...
          if (!response.ok) {
            console.error(`Erreur avec ${url} : ${response.statusText}`);
          } else {
            console.log("[Service Worker] Mise en cache des ressources :", url);
  ...
  event.respondWith(
    caches.match(event.request).then((response) => {
      ...
      return fetch(event.request)
        ...
        .catch((e) => {
          console.error(`Échec de récupération pour ${event.request.url} :`, e);
        });
    })
  );
});
